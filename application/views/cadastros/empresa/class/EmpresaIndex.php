...
class EmpresaIndex
{
    ...
    {
        $this->criterio   = "%";
        $this->sql        = "SELECT * FROM empresa WHERE ";
        $this->sql_count  = "SELECT COUNT(empresa_id) FROM empresa WHERE ";
        $this->field_sort = "empresa_data_cadastro";
        ...
        if ($this->permission['negar'] != 1) {
            $module = $this->security->encondeGET("Empresa");
    ...
    private function getCriteria()
    {
        ...
        if (strtoupper($this->criterio) == 'ATIVO') {
            $sql = $this->sql;
            $sql .= "(empresa_status = 1) ";
            $this->sql_count .= "(empresa_status = 1) ";
            $flag = 1;
        }elseif (strtoupper($this->criterio) == 'INATIVO') {
            $sql = $this->sql;
            $sql .= "(empresa_status = 0) ";
            $this->sql_count .= "(empresa_status = 0) ";
            $flag = 1;
        }
        //
            ...
            for ($i = 0; $i < $cols; $i++) {
                ...
                switch ($fld['name']) {
                    case 'empresa_nome':
                        ...
                        $flag = strtoupper($rs['empresa_status']);
                        //
                        if (!$flag) {
                            $cor = $COR;
                        } else {
                        ...
                        //
                        $b .= $this->generateButton($rs['empresa_id'], $rs['empresa_status']);
                        $b .= '</td>';
    ...
    private function createTheader()
    {
        ...
        $t .= $this->createSort('NOME', 'empresa_nome');
        $t .= $this->createSort('EMAIL', 'empresa_email');
        $t .= $this->createSort('CNPJ', 'empresa_cnpj');
        $t .= '<th nowrap>FONE 1</th>';
        $t .= $this->createSort('ID', 'empresa_id');
        $t .= '</tr>';
        $t .= '</thead>';
        return $t;
    }
    ...
    private function createSort($caption, $campoTabela)
    {
        ...
        //
        return '<th nowrap ' . $cor . '><a ' . $cor . ' href="empresa/' . $tmp . '">' . $caption . ' ' . $icon . '</a></th>';
    }
    ...
    private function generateButton($id, $active)
    {
        $ret = 0;
        $b   = "";
        $url = 'javascript:Empresa.showModal(' . $id . ');';
        //
        ...
        if ($this->permission['excluir'] == 1) {
            if (strtoupper($this->sid->getNode('empresa_id')) == strtoupper($id)) {
                $b .= $disabled;
            } else {
                if ($ret == 0) {
                    $b .= '<a href="javascript:;" onclick="Empresa.delReg(' . $id . ');void(0);" ';
