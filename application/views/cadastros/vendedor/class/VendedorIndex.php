...
class VendedorIndex
{
    ...
    {
        $this->criterio   = "%";
        $this->sql        = "SELECT * FROM vendedor WHERE ";
        $this->sql_count  = "SELECT COUNT(vendedor_id) FROM vendedor WHERE ";
        $this->field_sort = "vendedor_data_cadastro";
        ...
        if ($this->permission['negar'] != 1) {
            $module = $this->security->encondeGET("Vendedor");
            ...
            for ($i = 0; $i < $cols; $i++) {
                ...
                switch ($fld['name']) {
                    case 'vendedor_nome':
                        ...
                        $flag = strtoupper($rs['vendedor_status']);
                        //
                        if (!$flag) {
                            $cor = $COR;
                        } else {
                        ...
                        //
                        $b .= $this->generateButton($rs['vendedor_id'], $rs['vendedor_status']);
                        $b .= '</td>';
    ...
    private function createTheader()
    {
        ...
        $t .= $this->createSort('NOME', 'vendedor_nome');
        $t .= $this->createSort('PESSOA', 'vendedor_pessoa');
        $t .= $this->createSort('EMAIL', 'vendedor_email');
        $t .= '<th nowrap>FONE 1</th>';
        $t .= $this->createSort('ID', 'vendedor_id');
        $t .= '</tr>';
        $t .= '</thead>';
        return $t;
    }
    ...
    private function createSort($caption, $campoTabela)
    {
        ...
        //
        return '<th nowrap ' . $cor . '><a ' . $cor . ' href="vendedor/' . $tmp . '">' . $caption . ' ' . $icon . '</a></th>';
    }
    ...
    private function generateButton($id, $active)
    {
        $ret = 0;
        $b   = "";
        $url = 'javascript:Vendedor.showModal(' . $id . ');';
        //
        ...
        if ($this->permission['excluir'] == 1) {
            if (strtoupper($this->sid->getNode('vendedor_id')) == strtoupper($id)) {
                $b .= $disabled;
            } else {
                if ($ret == 0) {
                    $b .= '<a href="javascript:;" onclick="Vendedor.delReg(' . $id . ');void(0);" ';
