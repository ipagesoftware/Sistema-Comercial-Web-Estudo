...
class ClienteIndex
{
    ...
    public function __construct()
    {
        ...
        $this->sql_count  = "SELECT COUNT(cliente_id) FROM cliente WHERE ";
        $this->field_sort = "cliente_data_cadastro";
        ...
        if ($this->permission['negar'] != 1) {
            $module = $this->security->encondeGET("Cliente");
            ...
            for ($i = 0; $i < $cols; $i++) {
                ...
                switch ($fld['name']) {
                    case 'cliente_nome':
                        ...
                        $flag = strtoupper($rs['cliente_status']);
                        //
                        if (!$flag) {
                            $cor = $COR;
                        } else {
                        ...
                        //
                        $b .= $this->generateButton($rs['cliente_id'], $rs['cliente_status']);
                        $b .= '</td>';
    ...
    private function createTheader()
    {
        ...
        $t .= $this->createSort('NOME', 'cliente_nome');
        $t .= $this->createSort('PESSOA', 'cliente_pessoa');
        $t .= $this->createSort('EMAIL', 'cliente_email');
        $t .= '<th nowrap>FONE 1</th>';
        $t .= $this->createSort('ID', 'cliente_id');
        $t .= '</tr>';
        $t .= '</thead>';
        return $t;
    }
    ...
    private function createSort($caption, $campoTabela)
    {
        ...
        //
        return '<th nowrap ' . $cor . '><a ' . $cor . ' href="cliente/' . $tmp . '">' . $caption . ' ' . $icon . '</a></th>';
    }
    ...
    private function generateButton($id, $active)
    {
        $ret = 0;
        $b   = "";
        $url = 'javascript:Cliente.showModal(' . $id . ');';
        //
        ...
        if ($this->permission['excluir'] == 1) {
            if (strtoupper($this->sid->getNode('cliente_id')) == strtoupper($id)) {
                $b .= $disabled;
            } else {
                if ($ret == 0) {
                    $b .= '<a href="javascript:;" onclick="Cliente.delReg(' . $id . ');void(0);" ';
