...
class FabricanteIndex
{
    ...
    {
        $this->criterio  = "%";
        $this->sql       = "SELECT * FROM fabricante WHERE ";
        $this->sql_count = "SELECT COUNT(*) FROM fabricante WHERE ";
        //
        $this->field_sort = "fabricante_data_cadastro";
    ...
    private function getCriteria()
    {
        ...
        if (strtoupper($this->criterio) == 'ATIVO') {
            $sql = $this->sql;
            $sql .= "(fabricante_status = 1) ";
            $this->sql_count .= "(fabricante_status = 1) ";
            $flag = 1;
        } elseif (strtoupper($this->criterio) == 'INATIVO') {
            $sql = $this->sql;
            $sql .= "(fabricante_status = 0) ";
            $this->sql_count .= "(fabricante_status = 0) ";
            $flag = 1;
        }
        //
    ...
    public function insertDataIntoTable($criterio = '')
    {
        ...
        while ($rs = $query->fetch(PDO::FETCH_BOTH)) {
            for ($i = 0; $i < $cols; $i++) {
                ...
                switch ($fld['name']) {
                    case 'fabricante_descricao':
                        $flag = strtoupper($rs['fabricante_status']);
                        //
                        if (!$flag) {
                            $cor = $COR;
                        } else {
                        ...
                        $b .= $this->generateButton($rs['fabricante_id'], $rs['fabricante_status']);
                        $b .= '</td>';
    ...
    private function createTheader()
    {
        ...
        $t .= $this->createSort('DESCRIÇÃO', 'fabricante_descricao');
        $t .= $this->createSort('ID', 'fabricante_id');
        $t .= '  </tr>';
        $t .= '</thead>';
        return $t;
    }
    ...
    private function createSort($caption, $campoTabela)
    {
        ...
        //
        return '<th nowrap ' . $cor . '><a ' . $cor . ' href="fabricante/' . $tmp . '">' . $caption . ' ' . $icon . '</a></th>';
    }
    ...
    private function generateButton($id, $active)
    {
        ...
        if ($this->permission['excluir'] == 1) {
            if (strtoupper($this->sid->getNode('fabricante_id')) == strtoupper($id)) {
                $b .= $disabled;
            } else {
                if ($ret == 0) {
                    $b .= '<a href="javascript:;" onclick="Fabricante.delReg(' . $id . ');void(0);" ';
        ...
        if ($this->permission['editar'] == 1) {
            if ($ret == 0) {
                $b .= '&nbsp;<a href="fabricante/addupdt/' . $tmp . '" ';
