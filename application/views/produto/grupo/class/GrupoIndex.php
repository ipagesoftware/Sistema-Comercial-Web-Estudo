...
class GrupoIndex
{
    ...
    {
        $this->criterio  = "%";
        $this->sql       = "SELECT * FROM grupo WHERE ";
        $this->sql_count = "SELECT COUNT(*) FROM grupo WHERE ";
        //
        $this->field_sort = "grupo_data_cadastro";
    ...
    private function getCriteria()
    {
        ...
        if (strtoupper($this->criterio) == 'ATIVO') {
            $sql = $this->sql;
            $sql .= "(grupo_status = 1) ";
            $this->sql_count .= "(grupo_status = 1) ";
            $flag = 1;
        } elseif (strtoupper($this->criterio) == 'INATIVO') {
            $sql = $this->sql;
            $sql .= "(grupo_status = 0) ";
            $this->sql_count .= "(grupo_status = 0) ";
            $flag = 1;
        }
        //
            ...
            for ($i = 0; $i < $cols; $i++) {
                ...
                switch ($fld['name']) {
                    case 'grupo_descricao':
                        $flag = strtoupper($rs['grupo_status']);
                        //
                        if (!$flag) {
                            $cor = $COR;
                        } else {
                        ...
                        $b .= $this->generateButton($rs['grupo_id'], $rs['grupo_status']);
                        $b .= '</td>';
    ...
    private function createTheader()
    {
        ...
        $t .= $this->createSort('DESCRIÇÃO', 'grupo_descricao');
        $t .= $this->createSort('ID', 'grupo_id');
        $t .= '  </tr>';
        $t .= '</thead>';
        return $t;
    }
    ...
    private function createSort($caption, $campoTabela)
    {
        ...
        //
        return '<th nowrap ' . $cor . '><a ' . $cor . ' href="grupo/' . $tmp . '">' . $caption . ' ' . $icon . '</a></th>';
    }
    ...
    private function generateButton($id, $active)
    {
        ...
        if ($this->permission['excluir'] == 1) {
            if (strtoupper($this->sid->getNode('grupo_id')) == strtoupper($id)) {
                $b .= $disabled;
            } else {
                if ($ret == 0) {
                    $b .= '<a href="javascript:;" onclick="Grupo.delReg(' . $id . ');void(0);" ';
