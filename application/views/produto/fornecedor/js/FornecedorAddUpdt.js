...
$(document).ready(function() {
    FornecedorAddUpdt.init();
});
var FornecedorAddUpdt = function() {
    // variáveis privadas
    ...
    var handleTab = function() {
        ...
        $('form #tabs a').click(function(e) {
            ...
            if (aba === '#TAB_1_1') {
                $('#fornecedor_fone1').focus();
            } else if (aba === '#TAB_1_2') {
                $('#fornecedor_cpf').focus();
            } else if (aba === '#TAB_1_3') {
                $('#fornecedor_razao_social').focus();
            } else {
                $('#fornecedor_obs').focus();
            }
        });
    };
    /**
    ...
    function verificaCampos() {
        ...
        if (typeof(warning[0]) !== 'undefined') {
            ...
            ipageViews.notyMessage(msg, 'error', 0, function(result) {
                if (id == '#fornecedor_cpf') {
                    // Se o cpf for inválido, muda a aba 
                    // para pessoa física
                    $('#tabs a[href="#tab_1_2"]').tab('show');
                } else if (id == '#fornecedor_cnpj') {
    ...
    var save = function() {
        jQuestion('Confirma os dados digitados?', 'Atenção', function(r) {
            if (r) {
                var url = "application/views/produto/fornecedor/ajax/fornecedor_addupdt_ajax.php";
                ...
                $.ajax({
                    ...
                    success: function(txt, textStatus) {
                        ...
                        try {
                            ...
                            msg = txt;
                            id = 'fornecedor_nome'; // Primeira caixa de texto
                        }
                        if (msg == "OK") {
    ...
    return {
        init: function() {
            ...
            $("#form1").attr("action", "javascript:FornecedorAddUpdt.submitForm();void(0);");
