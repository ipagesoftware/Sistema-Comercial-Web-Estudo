...
class UmIndex
{
    ...
    {
        $this->criterio   = "%";
        $this->sql        = "SELECT * FROM um WHERE ";
        $this->sql_count  = "SELECT COUNT(*) FROM um WHERE ";
        $this->field_sort = "um_data_cadastro";
    ...
    public function atvDtv()
    {
        ...
        if ($this->permission['editar'] != 1) {
            $module = $this->security->encondeGET("Unidade de Medida");
            $r      = $this->security->encondeGET(rand(1000, 5000));
            header("Location: " . URL . "3109.php?modulo=" . $module . "&parameter=" . $r);
            die();
        }
    ...
    private function getCriteria()
    {
        ...
        if (strtoupper($this->criterio) == 'ATIVO') {
            $sql = $this->sql;
            $sql .= "(um_status = 1) ";
            $this->sql_count .= "(um_status = 1) ";
            $flag = 1;
        } elseif (strtoupper($this->criterio) == 'INATIVO') {
            $sql = $this->sql;
            $sql .= "(um_status = 0) ";
            $this->sql_count .= "(um_status = 0) ";
            $flag = 1;
        }
        //
            ...
            for ($i = 0; $i < $cols; $i++) {
                ...
                switch ($fld['name']) {
                    case 'um_descricao':
                        ...
                        $b .= $this->generateButton($rs['um_id'], $rs['um_status']);
                        $b .= '</td>';
                        $b .= '<td nowrap>' . pintaTextoConsulta(utf8_decode($rs[$fld['name']]), $this->criterio) . '</td>';
    ...
    private function createTheader()
    {
        ...
        $t .= $this->createSort('SIGLA', 'um_sigla');
        $t .= $this->createSort('DESCRIÇÃO', 'um_descricao');
        $t .= $this->createSort('ID', 'um_id');
        $t .= '  </tr>';
        $t .= '</thead>';
        return $t;
    }
    ...
    private function createSort($caption, $campoTabela)
    {
        ...
        //
        return '<th nowrap ' . $cor . '><a ' . $cor . ' href="um/' . $tmp . '">' . $caption . ' ' . $icon . '</a></th>';
    }
    ...
    private function generateButton($id, $active)
    {
        ...
        if ($this->permission['excluir'] == 1) {
            if (strtoupper($this->sid->getNode('um_id')) == strtoupper($id)) {
                $b .= $disabled;
            } else {
                if ($ret == 0) {
                    $b .= '<a href="javascript:;" onclick="Um.delReg(' . $id . ');void(0);" ';
